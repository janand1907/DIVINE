...
export async function generateMetadata(): Promise<Metadata> {
  const { theme, site, seoPage } = await fetchSeoContext('/divine-tours');
  return buildMetadata({ path: '/divine-tours', theme, site, seoPage });
}

export default async function DivineToursPage() {
      ...
      <section className="bg-brand-dark py-12 text-brand-darkForeground">
        <div className="container-brand">
          ...
          <p className="mt-2 max-w-2xl text-brand-darkForeground/80">
            Sacred pilgrimage journeys across India.
          </p>
        </div>
      </section>
          ...
          {destinations.length === 0 ? (
            <div className="rounded-lg border border-border bg-card p-12 text-center shadow-brand">
              <h2 className="font-heading text-xl font-semibold text-foreground">
                No divine destinations available yet
              </h2>
              <p className="mt-2 text-muted-foreground">
                Please check back soon for our curated pilgrimage tours.
              </p>
              <Link
                href="/packages"
