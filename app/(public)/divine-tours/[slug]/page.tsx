...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { theme, site, seoPage } = await fetchSeoContext(`/divine-tours/${params.slug}`);
  return buildMetadata({
    path: `/divine-tours/${params.slug}`,
...
export default async function DivineTourDetailPage({ params }: Props) {
  const supabase = createPublicClient();
  const { data: dest } = await supabase
    .from('destinations')
    .select('*')
  ...
  const { theme, site } = await fetchSeoContext(`/divine-tours/${params.slug}`);
  ...
  if (finalPackages.length === 0) {
    console.warn('[divine-tours/slug] OR filter returned no rows; falling back to destination_id-only');
  ...
  const cover = dest.cover_image ?? 'https://images.pexels.com/photos/3596389/pexels-photo-3596389.jpeg?auto=compress&cs=tinysrgb&w=1600';
            ...
            <div className="container-brand py-16 md:py-24">
              <div className="max-w-3xl">
                <Breadcrumb
                  items={[
                    { name: 'Home', href: '/' },
                    { name: 'Divine Tours', href: '/divine-tours' },
                    { name: dest.name, href: `/divine-tours/${dest.slug}` },
                  ]}
                  siteUrl={siteUrl}
          ...
          <section className="bg-background py-12">
            <div className="container-brand">
              <SectionHeading
                ...
                subtitle="Curated pilgrimage packages for this destination"
              />
