...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { theme, site, seoPage } = await fetchSeoContext(`/international-tours/${params.slug}`);
  return buildMetadata({
    path: `/international-tours/${params.slug}`,
...
export default async function InternationalTourDetailPage({ params }: Props) {
  ...
  const { theme, site } = await fetchSeoContext(`/international-tours/${params.slug}`);
  ...
  if (finalPackages.length === 0) {
    console.warn('[international-tours/slug] OR filter returned no rows; falling back to destination_id-only');
  ...
  const cover = dest.cover_image ?? 'https://images.pexels.com/photos/2026324/pexels-photo-2026324.jpeg?auto=compress&cs=tinysrgb&w=1600';
          ...
          <div className="max-w-3xl">
            <Breadcrumb
              items={[
                ...
                { name: dest.name, href: `/international-tours/${dest.slug}` },
              ]}
              siteUrl={siteUrl}
      ...
      <section className="bg-background py-12">
        <div className="container-brand">
          <SectionHeading
            ...
            subtitle="Curated international travel packages for this destination"
          />
