...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await supabase.from('vehicles').select().eq('id', params.id).single<VehicleRow>();
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const supabase = createAdminClient();
  const { data, error } = await supabase.from('vehicles').update(row).eq('id', params.id).select().single<VehicleRow>();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  await logActivity({ action: 'update', entity: 'vehicle', entityId: data.id, metadata: { slug: data.slug }, userEmail: session.email });
  return NextResponse.json(data);
}
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await logActivity({ action: 'delete', entity: 'vehicle', entityId: params.id, metadata: null, userEmail: session.email });
