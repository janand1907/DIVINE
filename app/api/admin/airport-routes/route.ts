...
export async function GET() {
  ...
  const { data, error } = await supabase.from('airport_routes').select().order('from_city');
...
export async function POST(req: NextRequest) {
  ...
  const supabase = createAdminClient();
  const { data, error } = await supabase.from('airport_routes').insert(row).select().single<AirportRouteRow>();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
